...
export default async function CustomerDetailPage({
  ...
  const agentName = (await getAgentName()) || "Agent";
  ...
  const [callLogs, orders] = await Promise.all([
    ...
    customer.websiteData
      ? getExistingCustomerOrders(customer.websiteData.$id)
      : Promise.resolve([]),
  ]);
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                <span
                  className={`text-xs px-2 py-0.5 rounded-full font-medium ${sourceBadge}`}
                >
        ...
        <div className="bg-white border rounded-lg p-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="ml-auto text-sm text-slate-500">
              {ordersWithDetails.length} orders
            </span>
          </div>

          {ordersWithDetails.length === 0 ? (
            <p className="text-slate-500 text-sm py-4">
              No orders from website yet.
            </p>
          ) : (
                ...
                {callLogs.map((log) => (
                  <div key={log.$id} className="border rounded-md p-3">
                    <div className="flex items-center justify-between mb-1">
                      <span
                        className={`text-xs px-2 py-0.5 rounded-full border ${getResponseTypeColor(log.response_type)}`}
                      >
                    ...
                    {log.notes && (
                      <p className="text-sm text-slate-700 mt-1">{log.notes}</p>
                    )}
                    {log.follow_up_date && (
                      <p className="text-xs text-amber-600 mt-1">
                        Follow-up:{" "}
                        {new Date(log.follow_up_date).toLocaleString()}
                      </p>
                    )}
                    {log.order_placed && (
