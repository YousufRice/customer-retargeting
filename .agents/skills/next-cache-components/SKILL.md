...
tsThis replaces the old `experimental.ppr` flag.
...
tsxBuilt-in profiles: `'default'`, `'minutes'`, `'hours'`, `'days'`, `'weeks'`, `'max'`
...
tsx### `updateTag()` - Immediate Invalidation
...
tsx### `revalidateTag()` - Background Revalidation
...
tsx### Exception: `use cache: private`
